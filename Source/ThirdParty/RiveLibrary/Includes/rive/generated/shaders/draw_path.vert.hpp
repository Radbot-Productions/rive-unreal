...
#ifdef EXPORTED_NEVER_GENERATE_PREMULTIPLIED_PAINT_COLORS
...
#elif defined(EXPORTED_ENABLE_ADVANCED_BLEND)
...
#define GENERATE_PREMULTIPLIED_PAINT_COLORS  !EXPORTED_ENABLE_ADVANCED_BLEND
#else
...
#elif !defined(EXPORTED_RENDER_MODE_MSAA)
#ifdef EXPORTED_DRAW_INTERIOR_TRIANGLES
EXPORTED_OPTIONALLY_FLAT VARYING(1, half, v_windingWeight);
#else
NO_PERSPECTIVE VARYING(2, COVERAGE_TYPE, v_coverages);
#endif //@DRAW_INTERIOR_TRIANGLES
EXPORTED_OPTIONALLY_FLAT VARYING(3, half, v_pathID);
#endif // !@RENDER_MODE_MSAA

#ifdef EXPORTED_ENABLE_CLIPPING
#ifdef EXPORTED_ATLAS_BLIT
EXPORTED_OPTIONALLY_FLAT VARYING(4, half, v_clipID); // [clipID, outerClipID]
#else
EXPORTED_OPTIONALLY_FLAT VARYING(4, half2, v_clipIDs); // [clipID, outerClipID]
#endif
...
EXPORTED_OPTIONALLY_FLAT VARYING(6, half, v_blendMode);
#endif

#ifdef EXPORTED_RENDER_MODE_CLOCKWISE_ATOMIC
...
VERTEX_MAIN(EXPORTED_drawVertexMain, Attrs, attrs, _vertexID, _instanceID)
{
...
#elif defined(EXPORTED_DRAW_INTERIOR_TRIANGLES)
    vertexPosition = unpack_interior_triangle_vertex(EXPORTED_a_triangleVertex,
                                                     pathID
#ifdef EXPORTED_RENDER_MODE_MSAA
                                                     ,
                                                     pathZIndex
    ...
    float4 coverages;
    shouldDiscardVertex =
        !unpack_tessellated_path_vertex(EXPORTED_a_patchVertexData,
...
#if !defined(EXPORTED_ATLAS_BLIT) && !defined(EXPORTED_RENDER_MODE_MSAA)
...
#ifdef EXPORTED_FRAMEBUFFER_BOTTOM_UP
...
#if defined(EXPORTED_ENABLE_CLIPPING) && !defined(EXPORTED_ATLAS_BLIT)
    else if (EXPORTED_ENABLE_CLIPPING && paintType == CLIP_UPDATE_PAINT_TYPE)
    {
        half outerClipID =
...
#elif defined(EXPORTED_RENDER_MODE_CLOCKWISE_ATOMIC)
        uint4 coverageData =
            STORAGE_BUFFER_LOAD4(EXPORTED_pathBuffer, pathID * 4u + 3u);
                              ...
                              float coverage FRAGMENT_CONTEXT_DECL)
{
    ...
    else if (paint.w > -1.) // Is paint is a gradient (linear or radial)?
    {
        ...
        color =
            TEXTURE_SAMPLE_LOD(EXPORTED_gradTexture, gradSampler, float2(x, row), .0);
        color.w *= coverage;
    ...
    {
        half lod = -paint.w - 2.;
        color = TEXTURE_SAMPLE_DYNAMIC_LOD(EXPORTED_imageTexture,
                                           imageSampler,
                                           paint.xy,
                                           lod);
...
{
#ifdef EXPORTED_ENABLE_FEATHER
    if (EXPORTED_ENABLE_FEATHER && is_feathered_stroke(coverages))
...
{
#if defined(EXPORTED_ENABLE_FEATHER)
    if (EXPORTED_ENABLE_FEATHER && is_feathered_fill(coverages))
