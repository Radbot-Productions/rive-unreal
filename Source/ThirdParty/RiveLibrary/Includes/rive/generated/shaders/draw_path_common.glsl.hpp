...
#if defined(EXPORTED_ENABLE_FEATHER) || defined(EXPORTED_ATLAS_BLIT)
SAMPLER_LINEAR(FEATHER_TEXTURE_IDX, featherSampler)
#endif

#ifdef EXPORTED_FRAGMENT
FRAG_TEXTURE_BLOCK_BEGIN
TEXTURE_RGBA8(PER_FLUSH_BINDINGS_SET, GRAD_TEXTURE_IDX, EXPORTED_gradTexture);
#if defined(EXPORTED_ENABLE_FEATHER) || defined(EXPORTED_ATLAS_BLIT)
TEXTURE_R16F_1D_ARRAY(PER_FLUSH_BINDINGS_SET,
                      FEATHER_TEXTURE_IDX,
                      EXPORTED_featherTexture);
...
TEXTURE_R16F(PER_FLUSH_BINDINGS_SET, ATLAS_TEXTURE_IDX, EXPORTED_atlasTexture);
#endif
...
// clang-format off
#if defined(EXPORTED_RENDER_MODE_MSAA) && defined(EXPORTED_ENABLE_ADVANCED_BLEND) && !defined(EXPORTED_FIXED_FUNCTION_COLOR_OUTPUT)
// clang-format on
...
#if defined(EXPORTED_FRAGMENT) && defined(EXPORTED_ENABLE_FEATHER)
...
#if defined(EXPORTED_VERTEX) && defined(EXPORTED_DRAW_PATH)
                                               ...
                                               VERTEX_CONTEXT_DECL)
{
    ...
    TESSDATA4 tessVertexData =
        TEXEL_FETCH(EXPORTED_tessVertexTexture, tess_texel_coord(tessVertexIdx));
    ...
    uint4 contourData = STORAGE_BUFFER_LOAD4(EXPORTED_contourBuffer, contourID - 1u);
    ...
    float2x2 M = make_float2x2(
        uintBitsToFloat(STORAGE_BUFFER_LOAD4(EXPORTED_pathBuffer, outPathID * 4u)));
    uint4 pathData = STORAGE_BUFFER_LOAD4(EXPORTED_pathBuffer, outPathID * 4u + 1u);
    ...
    if (localVertexID != vertexIDOnContour)
    {
        ...
        TESSDATA4 replacementTessVertexData =
            TEXEL_FETCH(EXPORTED_tessVertexTexture,
            ...
            (contourIDWithFlags & (MIRRORED_CONTOUR_CONTOUR_FLAG | 0xffffu)))
        {
            ...
            if (isClosed)
            {
                ...
                tessVertexData = TEXEL_FETCH(EXPORTED_tessVertexTexture,
    ...
    if (strokeRadius != .0) // Is this a stroke?
    {
        ...
        if (joinType > ROUND_JOIN_CONTOUR_FLAG)
        {
            ...
            TESSDATA4 otherJoinData =
                TEXEL_FETCH(EXPORTED_tessVertexTexture, otherJoinTexelCoord);
    ...
    uint4 pathData2 = STORAGE_BUFFER_LOAD4(EXPORTED_pathBuffer, outPathID * 4u + 2u);
...
#if defined(EXPORTED_VERTEX) && defined(EXPORTED_DRAW_INTERIOR_TRIANGLES)
...
#if defined(EXPORTED_VERTEX) && defined(EXPORTED_ATLAS_BLIT)
INLINE float2
