...
ATTR(
    0,
    float4,
    EXPORTED_a_p0p1_); // End in '_' because D3D interprets the '1' as a semantic index.
ATTR(1, float4, EXPORTED_a_p2p3_);
ATTR(2, float4, EXPORTED_a_joinTan_and_ys); // [joinTangent, y, reflectionY]
#ifdef SPLIT_UINT4_ATTRIBUTES
ATTR(3, uint, EXPORTED_a_x0x1);
ATTR(4, uint, EXPORTED_a_reflectionX0X1);
ATTR(5, uint, EXPORTED_a_segmentCounts);
ATTR(6, uint, EXPORTED_a_contourIDWithFlags);
#else
ATTR(3,
     uint4,
     EXPORTED_a_args); // [x0x1, reflectionX0X1, segmentCounts, contourIDWithFlags]
#endif
ATTR_BLOCK_END
#endif
...
TEXTURE_R16F_1D_ARRAY(PER_FLUSH_BINDINGS_SET,
                      FEATHER_TEXTURE_IDX,
                      EXPORTED_featherTexture);
VERTEX_TEXTURE_BLOCK_END

SAMPLER_LINEAR(FEATHER_TEXTURE_IDX, featherSampler)

VERTEX_STORAGE_BUFFER_BLOCK_BEGIN
STORAGE_BUFFER_U32x4(PATH_BUFFER_IDX, PathBuffer, EXPORTED_pathBuffer);
STORAGE_BUFFER_U32x4(CONTOUR_BUFFER_IDX, ContourBuffer, EXPORTED_contourBuffer);
VERTEX_STORAGE_BUFFER_BLOCK_END

VERTEX_MAIN(EXPORTED_tessellateVertexMain, Attrs, attrs, _vertexID, _instanceID)
{
    // Each instance repeats twice. Once for normal patch(es) and once for
    // reflection(s).
    ATTR_UNPACK(_instanceID, attrs, EXPORTED_a_p0p1_, float4);
    ATTR_UNPACK(_instanceID, attrs, EXPORTED_a_p2p3_, float4);
    ATTR_UNPACK(_instanceID, attrs, EXPORTED_a_joinTan_and_ys, float4);
#ifdef SPLIT_UINT4_ATTRIBUTES
    ATTR_UNPACK(_instanceID, attrs, EXPORTED_a_x0x1, uint);
    ATTR_UNPACK(_instanceID, attrs, EXPORTED_a_reflectionX0X1, uint);
    ATTR_UNPACK(_instanceID, attrs, EXPORTED_a_segmentCounts, uint);
    ATTR_UNPACK(_instanceID, attrs, EXPORTED_a_contourIDWithFlags, uint);

    uint4 EXPORTED_a_args = uint4(EXPORTED_a_x0x1,
                          EXPORTED_a_reflectionX0X1,
                          EXPORTED_a_segmentCounts,
                          EXPORTED_a_contourIDWithFlags);
#else
    ATTR_UNPACK(_instanceID, attrs, EXPORTED_a_args, uint4);
#endif
    ...
    float2 p0 = EXPORTED_a_p0p1_.xy;
    float2 p1 = EXPORTED_a_p0p1_.zw;
    float2 p2 = EXPORTED_a_p2p3_.xy;
    float2 p3 = EXPORTED_a_p2p3_.zw;
    ...
    float y = isFirstSpan ? EXPORTED_a_joinTan_and_ys.z : EXPORTED_a_joinTan_and_ys.w;
    int x0x1 = int(isFirstSpan ? EXPORTED_a_args.x : EXPORTED_a_args.y);
#ifdef GLSL
    int x1up = x0x1 << 16;
    if (EXPORTED_a_args.z == 0xffffffffu)
    {
    ...
    // Unpack arguments.
    uint parametricSegmentCount = EXPORTED_a_args.z & 0x3ffu;
    uint polarSegmentCount = (EXPORTED_a_args.z >> 10) & 0x3ffu;
    uint joinSegmentCount = EXPORTED_a_args.z >> 20;
    uint contourIDWithFlags = EXPORTED_a_args.w;
    ...
    uint pathID =
        contourID > 0u
            ...
            ? STORAGE_BUFFER_LOAD4(EXPORTED_contourBuffer, max(contourID, 1u) - 1u).z
            : 0u;
    uint4 pathData = pathID != 0u
                         ? STORAGE_BUFFER_LOAD4(EXPORTED_pathBuffer, pathID * 4u + 1u)
                         : uint4(0u, 0u, 0u, 0u);
        ...
        0u)
    {
        ...
        float2x2 mat = make_float2x2(
            uintBitsToFloat(STORAGE_BUFFER_LOAD4(EXPORTED_pathBuffer, pathID * 4u)));
    ...
    {
        float2x2 joinTangents = float2x2(tangents[1], EXPORTED_a_joinTan_and_ys.xy);
        float joinTheta =
        ...
        v_joinArgs.xy = EXPORTED_a_joinTan_and_ys.xy;
...
FRAG_DATA_MAIN(TESSDATA4, EXPORTED_tessellateFragmentMain)
{
